...
import cn.synway.bigdata.midas.MidasArray;
...
import static cn.synway.bigdata.midas.response.ByteFragmentUtils.parseArray;


public class MidasResultSet extends AbstractResultSet {
    ...
    private List<MidasColumnInfo> columns;
    ...
    private final MidasStatement statement;
    ...
    public MidasResultSet(InputStream is, int bufferSize, String db, String table,
        boolean usesWithTotals, MidasStatement statement, TimeZone timeZone,
        MidasProperties properties) throws IOException
    {
        this.db = db;
        this.table = table;
        ...
        if (headerFragment == null) {
            throw new IllegalArgumentException("Midas response without column names");
        }
        String header = headerFragment.asString(true);
        if (header.startsWith("Code: ") && !header.contains("\t")) {
            is.close();
            throw new IOException("Midas error: " + header);
        }
        String[] cols = toStringArray(headerFragment);
        ByteFragment typesFragment = bis.next();
        if (typesFragment == null) {
            throw new IllegalArgumentException("Midas response without column types");
        }
        String[] types = toStringArray(typesFragment);
        columns = new ArrayList<MidasColumnInfo>(cols.length);
        for (int i = 0; i < cols.length; i++) {
            columns.add(MidasColumnInfo.parse(types[i], cols[i]));
        }
    }
    ...
    private void checkValues(List<MidasColumnInfo> columns, ByteFragment[] values,
        ByteFragment fragment) throws SQLException
    {
        if (columns.size() != values.length) {
            throw MidasExceptionSpecifier.specify(fragment.asString());
        }
    }
    ...
    List<MidasColumnInfo> getColumns() {
        return Collections.unmodifiableList(columns);
    }

    @Override
    public ResultSetMetaData getMetaData() throws SQLException {
        return new MidasResultSetMetaData(this);
    }
    ...
    public Array getArray(int columnIndex) throws SQLException {
        MidasColumnInfo colInfo = columns.get(columnIndex - 1);
        if (colInfo.getMidasDataType() != MidasDataType.Array) {
        ...
        return new MidasArray(colInfo.getArrayBaseType(), array);
    }
    ...
    public Long getTimestampAsLong(int colNum) {
        MidasColumnInfo info = columns.get(colNum - 1);
    ...
    public Date getDate(int columnIndex) throws SQLException {
        // date is passed as a string from Midas
    ...
    public Object getObject(int columnIndex) throws SQLException {
        try {
            ...
            MidasDataType chType = columns.get(columnIndex - 1).getMidasDataType();
    ...
    private String getColumnNamesString() {
        ...
        for (MidasColumnInfo info : columns) {
