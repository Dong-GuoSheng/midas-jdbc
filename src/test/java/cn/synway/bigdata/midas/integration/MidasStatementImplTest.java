...
import cn.synway.bigdata.midas.MidasExternalData;
...
public class MidasStatementImplTest {
    ...
    public void testExternalData() throws SQLException, UnsupportedEncodingException {
        MidasStatement stmt = connection.createStatement();
        ResultSet rs = stmt.executeQuery(
                "select UserName, GroupName " +
                        "from (select 'User' as UserName, 1 as GroupId) as g" +
                        "any left join groups using GroupId",
                null,
                Collections.singletonList(new MidasExternalData(
                        "groups",
    ...
    public void testResultSetWithExtremes() throws SQLException {
        ...
        MidasDataSource dataSource = MidasContainerForTest.newDataSource(properties);
    ...
    public void cancelTest_queryId_is_not_set() throws Exception {
        final MidasStatement firstStatement = dataSource.getConnection().createStatement();
        ...
        Thread thread = new Thread() {
            ...
            public void run() {
                try {
                    Map<MidasQueryParam, String> params = new EnumMap<>(MidasQueryParam.class);
                    params.put(MidasQueryParam.CONNECT_TIMEOUT, Long.toString(TimeUnit.MINUTES.toMillis(1)));
        ...
        Thread thread = new Thread() {
            ...
            public void run() {
                try {
                    Map<MidasQueryParam, String> params = new EnumMap<>(MidasQueryParam.class);
                    params.put(MidasQueryParam.CONNECT_TIMEOUT, Long.toString(TimeUnit.MINUTES.toMillis(1)));
                    params.put(MidasQueryParam.QUERY_ID, queryId);
