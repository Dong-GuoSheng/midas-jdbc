...
public class MidasSqlParserTest {
    private MidasSqlStatement[] parse(String sql) {
        return MidasSqlParser.parse(sql, new MidasProperties());
    }

    private String loadSql(String file) {
        InputStream inputStream = MidasSqlParserTest.class.getResourceAsStream("/sqls/" + file);
    ...
    private void checkSingleStatement(MidasSqlStatement[] stmts, String sql) {
        checkSingleStatement(stmts, sql, StatementType.UNKNOWN, MidasSqlStatement.DEFAULT_DATABASE,
                MidasSqlStatement.DEFAULT_TABLE);
    }

    private void checkSingleStatement(MidasSqlStatement[] stmts, String sql, StatementType stmtType) {
        checkSingleStatement(stmts, sql, stmtType, MidasSqlStatement.DEFAULT_DATABASE,
                MidasSqlStatement.DEFAULT_TABLE);
    }
    ...
    public void testParseNonSql() throws ParseException {
        ...
        Assert.assertEquals(parse(sql = null),
                new MidasSqlStatement[] { new MidasSqlStatement(sql, StatementType.UNKNOWN) });
    ...
    public void testInsertStatement() throws ParseException {
        ...
        MidasSqlStatement s = parse(sql = "insert into table test(a,b) Values (1,2)")[0];
    ...
    public void testSelectStatement() {
        ...
        Assert.assertEquals(parse(sql = "select\n1"), new MidasSqlStatement[] {
                new MidasSqlStatement(sql, StatementType.SELECT, null, null, "unknown", null, null, null, null) });
        Assert.assertEquals(parse(sql = "select\r\n1"), new MidasSqlStatement[] {
                new MidasSqlStatement(sql, StatementType.SELECT, null, null, "unknown", null, null, null, null) });

        Assert.assertEquals(parse(sql = "select 314 limit 5\nFORMAT JSONCompact;"),
                new MidasSqlStatement[] { new MidasSqlStatement("select 314 limit 5\nFORMAT JSONCompact",
        ...
        Assert.assertEquals(parse(sql = loadSql("issue-441_with-totals.sql")),
                new MidasSqlStatement[] { new MidasSqlStatement(sql, StatementType.SELECT, null, null,
        ...
        Assert.assertEquals(parse(sql = loadSql("issue-555_custom-format.sql")),
                new MidasSqlStatement[] { new MidasSqlStatement(sql, StatementType.SELECT, null, null, "wrd",
                        "CSVWithNames", null, null, null) });
        Assert.assertEquals(parse(sql = loadSql("with-clause.sql")), new MidasSqlStatement[] {
                new MidasSqlStatement(sql, StatementType.SELECT, null, null, "unknown", null, null, null, null) });
    }
    ...
    public void testMultipleStatements() throws ParseException {
        Assert.assertEquals(parse("use ab;;;select 1; ;\t;\r;\n"), new MidasSqlStatement[] {
                new MidasSqlStatement("use ab", StatementType.USE, null, "ab", null, null, null, null, null),
                new MidasSqlStatement("select 1", StatementType.SELECT) });
        Assert.assertEquals(parse("select * from \"a;1\".`b;c`;;;select 1 as `a ; a`; ;\t;\r;\n"),
                new MidasSqlStatement[] {
                        new MidasSqlStatement("select * from \"a;1\".`b;c`", StatementType.SELECT, null, "a;1",
                                "b;c", null, null, null, null),
                        new MidasSqlStatement("select 1 as `a ; a`", StatementType.SELECT) });
    }
    ...
    public void testFormat() throws ParseException {
        ...
        MidasSqlStatement[] stmts = parse(sql);
        assertEquals(stmts.length, 1);
    ...
    public void testParameterHandling() throws ParseException {
        ...
        stmts = MidasSqlParser.parse(sql, new MidasProperties(), new ParseHandler() {
            @Override
    ...
    static void parseAllSqlFiles(File f) throws IOException {
        if (f.isDirectory()) {
            ...
            MidasSqlParser p = new MidasSqlParser(sql.toString(), null, null);
            try {
                p.sql();
    ...
    // TODO: add a sub-module points to Midas/tests/queries?
